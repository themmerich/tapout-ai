...
**Note:** Signal Forms are part of Angular's public API as of v22 (imported from `@angular/forms/signals`). For existing apps built on Reactive Forms, see [references/form-patterns.md](references/form-patterns.md) for Reactive Forms patterns.
...
Use `debounce` in the schema to control how often UI input propagates to the model (and re-runs validators) — useful before async validation:

```typescript
import { debounce } from '@angular/forms/signals';

const loginForm = form(this.loginModel, (schemaPath) => {
  debounce(schemaPath.email, 500); // ms delay; also accepts 'blur' or a custom debouncer
